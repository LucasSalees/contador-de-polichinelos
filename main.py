...
class CompetitionSystem:
    def _init_(self, jogador1, jogador2, meta_polichinelos=20):
    ...
    def registrar_polichinelo(self, jogador):
        ...
        if jogador == 1:
            self.contador1 += 1
            if self.contador1 >= self.meta_polichinelos and not self.vencedor:
                self.vencedor = 1
                self.tempo_final = time.time() - self.tempo_inicio
        else:
            self.contador2 += 1
            if self.contador2 >= self.meta_polichinelos and not self.vencedor:
                self.vencedor = 2
                self.tempo_final = time.time() - self.tempo_inicio
...
def detectar_multiplas_pessoas_corrigido(image, pose_model):
    ...
    if resultado_direita.pose_landmarks:
        nose = resultado_direita.pose_landmarks.landmark[0]
        left_shoulder = resultado_direita.pose_landmarks.landmark[11]
        right_shoulder = resultado_direita.pose_landmarks.landmark[12]
        
        centro_x = (nose.x + left_shoulder.x + right_shoulder.x) / 3.0
        
        if centro_x > 0.25:
            jogador2_original = resultado_direita.pose_landmarks
...
def validar_pose_melhorada(landmarks):
    """
    Validação mais eficiente e menos restritiva
    """
    if not landmarks:
        return False, 0.0
    ...
    visibilidades = []
    for idx in pontos_criticos:
        if idx < len(landmarks.landmark):
            vis = landmarks.landmark[idx].visibility
            visibilidades.append(vis)
    
    if not visibilidades:
        return False, 0.0
    
    vis_media = sum(visibilidades) / len(visibilidades)
    vis_minima = min(visibilidades)
    
    pose_valida = vis_media >= 0.35 and vis_minima >= 0.25
    
    return pose_valida, vis_media

def detectar_postura_polichinelo_competicao(landmarks, w, h):
    """
    Detecção de postura otimizada para modo competição
    """
    def pt(lm_id):
        lm = landmarks.landmark[lm_id]
        return (lm.x * w, lm.y * h, lm.visibility)
    
    l_sh_x, l_sh_y, l_sh_vis = pt(11)
    r_sh_x, r_sh_y, r_sh_vis = pt(12)
    l_hp_x, l_hp_y, l_hp_vis = pt(23)
    r_hp_x, r_hp_y, r_hp_vis = pt(24)
    l_wr_x, l_wr_y, l_wr_vis = pt(15)
    r_wr_x, r_wr_y, r_wr_vis = pt(16)
    l_an_x, l_an_y, l_an_vis = pt(27)
    r_an_x, r_an_y, r_an_vis = pt(28)
    l_el_x, l_el_y, l_el_vis = pt(13)
    r_el_x, r_el_y, r_el_vis = pt(14)
    
    shoulder_mid_y = (l_sh_y + r_sh_y) / 2.0
    hip_mid_y = (l_hp_y + r_hp_y) / 2.0
    shoulder_width = abs(r_sh_x - l_sh_x)
    ...
    arms_up = (wrist_mid_y < shoulder_mid_y + tolerance_up and 
               elbow_mid_y < shoulder_mid_y + tolerance_up and
               l_wr_vis > 0.3 and r_wr_vis > 0.3)
    
    arms_down = (wrist_mid_y > hip_mid_y - tolerance_down and
                 l_wr_vis > 0.3 and r_wr_vis > 0.3)
    
    ankle_distance = abs(r_an_x - l_an_x)
    
    if shoulder_width > 20:
        normalized_distance = ankle_distance / shoulder_width
    else:
        normalized_distance = 0
    
    legs_open = (normalized_distance > 1.3 and 
                 l_an_vis > 0.3 and r_an_vis > 0.3)
    
    legs_closed = (normalized_distance < 1.5 and
                   l_an_vis > 0.3 and r_an_vis > 0.3)
...
def processar_jogador_competicao(landmarks, landmarks_original, w, h, jogador_num, 
    ...
    if not pose_valida:
        return (counter, stage, max(0, open_frames - 1), max(0, closed_frames - 1), 
                flash_frames, f"Pose invalida (vis: {vis_media:.2f})")
    
    arms_up, legs_open, arms_down, legs_closed = detectar_postura_polichinelo_competicao(
        landmarks_original or landmarks, w, h
    )
    ...
    if arms_up and legs_open:
        ...
        if open_frames > 15:
            open_frames = 15
        status = f"ABERTO (frames: {open_frames})"
        ...
        if closed_frames > 15:
            closed_frames = 15
        status = f"FECHADO (frames: {closed_frames})"
    ...
    if stage == "down" and open_frames >= threshold:
        ...
        flash_frames = 15
        status += " -> POLICHINELO!"
        print(f"[Frame {frame_atual}] JOGADOR {jogador_num}: Polichinelo #{counter} completo!")
...
def detectar_postura_polichinelo(landmarks, w, h):
    ...
    hip_mid_y = (l_hp_y + r_hp_y) / 2.0
    shoulder_width = abs(r_sh_x - l_sh_x)
    hip_width = abs(r_hp_x - l_hp_x)
    
    body_width = max(shoulder_width, hip_width, 50.0)
